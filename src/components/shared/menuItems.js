...
const MenuItems = ({sidebarToggle, buttons, title}) => {
  ...
  return (
    <div
      className={`${sidebarToggle ? "p-[18px]" : "p-6"} flex flex-col gap-3.5`}
    >
      {!sidebarToggle && (
        <h3 className="text-[#9298B4] text-xs font-bold mb-0.5">{title}</h3>
      )}
      {buttons.map((button, index) => (
        ...
        >
          <button
            ...
            className={`text-start text-sm font-semibold bg-transparent ${
